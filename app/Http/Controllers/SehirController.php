...
{
    // Tüm şehir verilerini tutan dizi
    private $sehirler = [
    ...
    {
        return view('sehir.index');
    }

    // /sehir/{plaka} sayfası
    public function show($plaka)
    {
        $sehir = $this->sehirler[$plaka] ?? null;

        if (!$sehir) {
            // Geçersiz plaka için hata sayfası göster
            return view('sehir.notfound', compact('plaka'));
        }

        return view('sehir.show', compact('plaka', 'sehir'));
    }
}
